...
async function getSession(phone) {
  ...
  const [rows] = await pool.query(
    'SELECT * FROM wa_sessions WHERE phone = ? AND updated_at > DATE_SUB(NOW(), INTERVAL 30 MINUTE)',
    [phone]
  );
  if (!rows.length) {
...
async function handle(phone, displayName, input, msgType) {
  ...
  const resetWords = ['menu', 'hi', 'hello', 'start', 'restart', 'hallo'];
...
async function sendWelcome(phone, name) {
  await sendButtons(
    ...
    [
      { id: 'svc_photo', title: '📸 Photography' },
      { id: 'svc_video', title: '🎬 Videography' },
      { id: 'svc_web',   title: '💻 Web Design' },
    ],
...
async function handleService(phone, input, data) {
  ...
  data.service       = input;
  data.service_label = SERVICES[input];
  const sections = [{ title: 'Choose a Type', rows: SUB_TYPES[input] }];
  await sendList(phone, `Great choice! 🙌\n\nWhat type of ${data.service_label} are you looking for?`, 'View Options', sections);
  await saveSession(phone, 'CHOOSE_SUBTYPE', data);
}

async function handleSubType(phone, input, data) {
  const subs = SUB_TYPES[data.service] || [];
  const sub  = subs.find(s => s.id === input);
  if (!sub) {
    await sendText(phone, 'Please select an option from the list, or type *menu* to restart.');
    return;
  }
  data.subtype       = input;
  data.subtype_label = sub.title;
...
async function handleDate(phone, input, data) {
  if (input.trim().length < 3) {
    await sendText(phone, 'Please enter a date or timeframe — e.g. _20 July_ or _first week of August_.');
    return;
  }
...
async function handleEmail(phone, input, data) {
  ...
  if (!emailRegex.test(input)) {
    await sendText(phone, "That doesn't look like a valid email. Please try again — e.g. _name@gmail.com_");
    return;
  }
  data.client_email = input;
  await sendText(phone, 'Almost done! Any extra details, location, or requirements?\n\n_Type *skip* if none._');
...
async function sendConfirmSummary(phone, data) {
  ...
  await sendButtons(phone, summary, [
    ...
    { id: 'confirm_edit',   title: '✏️ Start Over' },
  ...
  if (input === 'confirm_yes') {
    ...
    const [result] = await pool.query(
      `INSERT INTO wa_bookings (phone, client_name, client_email, service, service_label, subtype, subtype_label, preferred_date, notes, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())`,
      [phone, data.client_name, data.client_email, data.service, data.service_label,
       data.subtype, data.subtype_label, data.preferred_date, data.notes || '']
    );
    const bookingId = result.insertId;
    await sendText(phone,
      `🎉 *Booking Confirmed!*\n\n` +
      `Thanks *${data.client_name}* — we've received your request and will be in touch within 24 hours.\n\n` +
      `📋 *Reference:* #BMG-${bookingId}\n` +
      `📧 Confirmation sent to ${data.client_email}.\n\n` +
      `Type *menu* anytime to make another booking. 🙏`
    );
    await saveSession(phone, 'DONE', data);
  } else if (input === 'confirm_edit') {
    await resetSession(phone);
    await sendText(phone, "No problem! Type *hi* when you're ready to start over. 😊");
  } else if (input === 'confirm_cancel') {
    await resetSession(phone);
    await sendText(phone, "Booking cancelled. Type *hi* anytime to book with us! 👋");
  } else {
    await sendConfirmSummary(phone, data);
  }
}
...
app.get('/webhook', (req, res) => {
  ...
  if (hub_mode === 'subscribe' && hub_verify_token === WEBHOOK_VERIFY_TOKEN) {
...
app.get('/', (req, res) => res.send('BMG Booking Bot is running ✅'));
